...
mrm_dataset = load_dataset("sehyun66/News-sentiments",
                           "distill", split="default")
ahm_dataset = load_dataset('sehyun66/News-sentiments',
                           "bertplus", split='default')
nick_dataset = load_dataset(
    'sehyun66/News-sentiments', "debert", split='default')
pros_dataset = load_dataset(
    'sehyun66/News-sentiments', "finbert", split='default')
...
for idx, df in enumerate(sentiment_df_list):
    df['headline_label'] = df['headline_sentiment'].apply(
        lambda x: max(x, key=x.get))
    df['summary_label'] = df['summary_sentiment'].apply(
        lambda x: max(x, key=x.get))
    df['negative_sentiment'] = df['headline_sentiment'].apply(
        lambda x: float(x['negative']))


##데이터 전처리
document_list = []
for df in sentiment_df_list:
    negative_documents_headline = list(
        df[df['headline_label'] == 'negative']['headline'])
    postive_documents_headline = list(
        df[df['headline_label'] == 'postive']['headline'])
    negative_documents_summary = list(
        df[df['headline_label'] == 'negative']['summary'])
    postive_documents_summary = list(
        df[df['headline_label'] == 'postive']['summary'])
    diction = {}
